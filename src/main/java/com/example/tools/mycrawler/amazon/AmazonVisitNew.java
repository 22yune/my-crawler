 ...
 * @date 2019/6/25
 */
public class AmazonVisitNew {
    private static Logger logger = LoggerFactory.getLogger(AmazonVisitNew.class);
    ...
    public static Queue<Book> receiveBookInPage(String url, Consumer<Book> visit, AtomicBoolean parallel, int restry) {
        ...
        String select = "div#a-page div#zg div.a-fixed-left-flipped-grid div.a-fixed-left-grid-inner div#zg-right-col div#zg-center-div ol#zg-ordered-list li.zg-item-immersion span.a-list-item";
        Elements elements = getElements(getDocument(url,3),select);
    ...
    public static Queue<Category> receiveCategory(String url, final int level, int restry,Consumer<Category> visit, AtomicBoolean parallel) {
            ...
            re.select(root.toString()).forEach( element -> {
                ...
                if (elements != null && elements.size() > 0) {
                    ...
                    Consumer<Element> childAction = li -> {
                        ...
                        Queue<Category> categoriesChild = receiveCategory(href, level + 1, restry, visit, parallel);
                    ...
                    parallel(elements,childAction,parallel);
                }
            });
        });
        if(has.get()){
    ...
    public static ExecutorService newExecutor(int corePoolSize) {
        ...
        return new ThreadPoolExecutor(Math.min(20,corePoolSize), Math.max(Runtime.getRuntime().availableProcessors(), 20),
                ...
                (Runnable r) -> {
                    ...
                    a.setName("parallel" + index.getAndIncrement() + " " + Integer.toHexString(a.hashCode()));
                    return a;
                }
    ...
    public static void main(String[] args) {
        ...
        Queue<Category> list = receiveCategory("https://www.amazon.cn/gp/bestsellers/digital-text/143359071/ref=zg_bs_nav_kinc_2_116169071", 3, 1, categoryConsumer,new AtomicBoolean(true));
