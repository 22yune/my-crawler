...
public class GetBook4Recipe {
    ...
    public static List<Book> getDownloadUrl(UserContext user, String bookName, Set<String> bookNames, Supplier<String> tErr) {
        return getDownloadUrl(user, bookName, bookNames, tErr,false);
    }
    public static List<Book> getDownloadUrl(UserContext user, String bookName, Set<String> bookNames, Supplier<String> tErr,boolean recipe) {
                ...
                try {
                    ...
                    for (Book myBook : myBooks) {
                        ...
                        if (book.getTitle().equals(myBook.getTitle())) {
                            ...
                            try {
                                location = ReadBook.readBook(myBook.getReadUrl(), user,recipe);
                            } catch (TimeoutException e) {
                                break;
                            }
